...
export async function POST(request: Request) {
  ...
  const validationError = getHeaderBrandImageValidationError(imageFile);
  ...
  if (validationError === "size") {
    return NextResponse.json(
      { error: `The uploaded JPG must be ${Math.round(MAX_HEADER_BRAND_IMAGE_BYTES / 1024 / 1024)}MB or smaller.` },
      { status: 400 },
    );
  }
  ...
  const storageKey = buildContentImageStorageKey(imageFile.name, "header-brand");
