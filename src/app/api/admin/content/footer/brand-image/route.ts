...
export async function POST(request: Request) {
  ...
  const validationError = getFooterBrandImageValidationError(imageFile);
  if (validationError === "type") {
    return NextResponse.json({ error: "Only JPG images are allowed." }, { status: 400 });
  }

  if (validationError === "size") {
    return NextResponse.json(
      { error: `The uploaded JPG must be ${Math.round(MAX_FOOTER_BRAND_IMAGE_BYTES / 1024 / 1024)}MB or smaller.` },
      { status: 400 },
    );
  }
  ...
  const storageKey = buildContentImageStorageKey(imageFile.name, "footer-brand");
  ...
  return NextResponse.json({ imageUrl: buildContentImageUrl(storageKey) }, { status: 201 });
}
