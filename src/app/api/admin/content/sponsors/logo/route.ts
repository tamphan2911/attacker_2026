...
import {
  buildSponsorImageStorageKey,
  buildSponsorImageUrl,
  storeSponsorImageFile,
...
export async function POST(request: Request) {
  ...
  const storageKey = buildSponsorImageStorageKey(imageFile.name);
  const imageBuffer = Buffer.from(await imageFile.arrayBuffer());

  try {
    await storeSponsorImageFile(storageKey, imageBuffer);
  } catch {
    return NextResponse.json(
  ...
  return NextResponse.json(
    {
      imageUrl: buildSponsorImageUrl(storageKey),
    },
    { status: 201 },
  );
}
