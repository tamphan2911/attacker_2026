...
export async function POST(request: Request) {
  ...
  const storageKey = buildNewsImageStorageKey(imageFile.name);
  const imageBuffer = Buffer.from(await imageFile.arrayBuffer());

  try {
    await storeNewsImageFile(storageKey, imageBuffer);
  } catch {
    return NextResponse.json(
  ...
  return NextResponse.json(
    {
      imageUrl: buildNewsImageUrl(storageKey),
    },
    { status: 201 },
  );
}
