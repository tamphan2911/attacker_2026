...
import {
  buildJudgeImageStorageKey,
  buildJudgeImageUrl,
  storeJudgeImageFile,
} from "@/server/judge-image-storage";
...
export async function POST(request: Request) {
  ...
  const storageKey = buildJudgeImageStorageKey(imageFile.name);
  const imageBuffer = Buffer.from(await imageFile.arrayBuffer());

  try {
    await storeJudgeImageFile(storageKey, imageBuffer);
  } catch {
    return NextResponse.json(
  ...
  return NextResponse.json(
    {
      imageUrl: buildJudgeImageUrl(storageKey),
    },
    { status: 201 },
  );
}
