...
const ROUND2_AI_JUDGE_PERSPECTIVES = [
  {
    label: "Giám khảo GPT 1 - Góc nhìn kinh doanh/tài chính",
...
type Round2AiJudgePerspective = (typeof ROUND2_AI_JUDGE_PERSPECTIVES)[number];
...
async function scoreReportWithOpenAi({
  ...
  reportBuffer: Buffer;
  judgePerspective: Round2AiJudgePerspective;
}) {
      ...
      input: [
        {
          ...
          content: [
            ...
            "Không viết nhận xét cân bằng chung chung. Hãy giữ đúng vai chuyên môn được giao và dùng các tiêu chí trong rubric qua lăng kính chuyên môn đó.",
...
function buildDetailedComment(result: OpenAiReportScoringResult, judgePerspective: Round2AiJudgePerspective) {
  return [
    judgePerspective.label,
    "",
    result.comment,
    "",
    "Nhận xét theo rubric:",
...
async function countEligibleSubmissions(mode: Round2AiReportScoringJobMode, retryJobCreatedAt?: Date) {
...
async function findNextCandidate(mode: Round2AiReportScoringJobMode, retryJobCreatedAt?: Date) {
  const { submissions } = await readLatestRound2Submissions();
  return submissions
...
export async function processNextRound2AiReportScoringJobItem(
  ...
  const retryJobCreatedAt = job.mode === Round2AiReportScoringJobMode.RETRY_FAILED ? job.createdAt : undefined;
    ...
    const aiJudgeResults = await Promise.all(
      assignedJudgeReviews.map(async (_review, index) => {
        ...
        return {
          ...
          detailedComment: buildDetailedComment(result, perspective),
        };
      }),
    );
    const totalScore =
