...
export function OrganizerContent({
  ...
  const shiftGallery = (direction: -1 | 1) => {
    setActiveGalleryIndex(
      (current) =>
        (current + direction + gallerySlides.length) % gallerySlides.length,
    );
  };
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          {showSeasonIntroCopy ? (
            <SectionHeading
              eyebrow={pickText(
                locale,
                pageContent.organizer.contentModules.eyebrow,
              )}
              title={pickText(
                locale,
                pageContent.organizer.contentModules.title,
              )}
              description={pickText(
                locale,
                pageContent.organizer.contentModules.description,
              )}
            />
          ) : (
        ...
        <div className="grid gap-5 xl:grid-cols-2">
          {seasonStories.map((season) => {
            const seasonCardImage =
              season.featuredImage || season.image || "/theme-feature-1.jpg";

            return (
              <Surface
                key={season.year}
                className="h-full overflow-hidden px-0 py-0"
              >
                  ...
                  <div className="relative min-h-[260px] overflow-hidden lg:min-h-0 lg:self-stretch">
                    ...
                    >
                      <Image
                        ...
                        unoptimized={shouldUseUnoptimizedImage(seasonCardImage)}
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                aria-label={pickText(
                  locale,
                  organizerContent.previousPhotoLabel,
                )}
              >
            ...
            <div className="flex items-center justify-between gap-4 border-b border-white/10 px-5 py-4 md:px-6">
              <div className="min-w-0">
                <p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-200">
                  {expandedGallerySlide.year} ·{" "}
                  {pickText(locale, expandedGallerySlide.label)}
                </p>
              ...
              <button
                ...
                aria-label={pickText(
                  locale,
                  organizerContent.closeGalleryLabel,
                )}
              >
                <X className="h-5 w-5" />
              </button>
